...
# Connect and send the test data
python3 -c "
import socket, time
s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
s.settimeout(2)
s.connect(('127.0.0.1', $TEST_PORT))
s.sendall(b'Deferred submit test!')
time.sleep(1)
s.close()
"
...
echo "# DESCRIPTION: Batch submit deferred IO. cs.read() with set_submit(false), manual event_service->submit(). start.sh spawns test_program and sends data via python3 client. Verifies deferred read receives correct data."
echo "#####################################################"
