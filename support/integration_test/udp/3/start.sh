...
client_ip=$(grep 'IP = \[' test_program2/bin/result1 | awk -F 'IP = \\[' '{print $2}' | awk -F '\\]' '{print $1}' | sort -u)
RESULT_CNT3=$(echo $client_ip | wc -l)
echo "RESULT_CNT3 = $RESULT_CNT3"
client_port=$(grep 'PORT = \[' test_program2/bin/result1 | awk -F 'PORT = \\[' '{print $2}' | awk -F '\\]' '{print $1}' | sort -u)
RESULT_CNT4=$(echo $client_port | wc -l)
...
if [[ $RESULT_CNT1 -eq 0 && $RESULT_CNT2 -eq 10000 && $RESULT_CNT3 -eq 1 && $RESULT_CNT4 -eq 1 && "$client_ip" == "127.0.0.1" && "$client_port" == "12001" ]]; then
    echo "# RESULT     : PASS"
else
    echo "# RESULT     : FAILED"
fi
echo "# DESCRIPTION: Create a udp server and a udp client. Client sends 10000 random string to server. Check if server can receive correctly! Udp server sets need_read_detail_ = true, it reads and prints client's ip and port, Check if the received ip and port is equal to client's real ip and port."
echo "#####################################################"
